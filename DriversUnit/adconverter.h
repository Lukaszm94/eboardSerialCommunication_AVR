...
public:
	...
	uint16_t  getRawMeasurement(uint8_t channel)
	{
		setChannel(channel);
		startConversion();
	...
	uint32_t getRawSum(uint8_t channel, int samplesCount, int16_t offset = 0) 
	{
		uint32_t sum = 0;
	...
	float getAverageVoltage(uint8_t channel, int numberOfSamples) {
		unsigned long sum = 0;
...
private:
	...
	inline bool isCoversionFinished()
	{
		return !(ADCSRA & (1<<ADSC)); //checking AD Start Conversion flag, which is set to 1 if conversion is pending
	}
	float convertRawMeasurementToVoltage(uint16_t rawReading)
	{
		return (float)rawReading/maxValue*referenceVoltage;
	}
